...
// Définition du composant fonctionnel AUserEdit
export default function AUserEdit() {
  ...
  useEffect(() => {
    ...
    fetchUser(); // Appel de la fonction fetchUser au montage du composant
  }, [id]); // Le tableau dépendance indique que cette action dépend de l'ID et sera re-exécutée si l'ID change
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      await axios.put(`http://localhost:5000/users/${id}`, editedUser);
    ...
    <div className="Section_User_Edit">
      <div className="A_User_Edit">
        ...
        <div className="section_input_group">
          ...
          <div className="input-group">
            <label htmlFor="lvlDePadel">Niveau de Padel:</label>
            <input
              id="lvlDePadel"
              type="number"
              name="niveauPadel"
